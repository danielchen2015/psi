...
class InventoryController extends PSIBaseController {
	 ...
	 * 库存总账 - SKU细化到保质期
	 */
	public function inventoryQcList() {
		if (IS_POST) {
			$params = [
					"warehouseId" => I("post.warehouseId"),
					"code" => I("post.code"),
			...
			$this->ajaxReturn((new InventoryService())->inventoryQcList($params));
		}
	}
	 ...
	 * 库存明细账 - SKU细化到保质期
	 */
	public function inventoryQcDetailList() {
		if (IS_POST) {
			$params = array(
					"warehouseId" => I("post.warehouseId"),
					"goodsId" => I("post.goodsId"),
			...
			$is = new InventoryService();
			$this->ajaxReturn($is->inventoryQcDetailList($params));
		}
	}
}
