...
class PurchaseController extends PSIBaseController {
	 ...
	 * 采购订单编辑页面，选择采购模板主表列表
	 */
	public function selectPCTemplateList() {
		if (IS_POST) {
			$params = [
					"ref" => I("post.ref"),
					"supplierId" => I("post.supplierId"),
			...
			$service = new POBillService();
			$this->ajaxReturn($service->selectPCTemplateList($params));
		}
	}

	/**
	 * 采购订单编辑页面，选择采购模板物资明细列表
	 */
	public function pcTemplateDetailListForPOBill() {
		if (IS_POST) {
			$params = [
					"id" => I("post.id")
			];
			...
			$service = new POBillService();
			$this->ajaxReturn($service->pcTemplateDetailListForPOBill($params));
		}
	}
	 ...
	 */
	public function getPCTemplateInfoForPOBill() {
		if (IS_POST) {
			$params = [
					"id" => I("post.id")
			];
			...
			$service = new POBillService();
			$this->ajaxReturn($service->getPCTemplateInfoForPOBill($params));
		}
	}
	 ...
	 * 采购入库单中选择采购订单列表
	 */
	public function selectPOBillListForPWBill() {
		if (IS_POST) {
			$params = [
					"ref" => I("post.ref"),
					"supplierId" => I("post.supplierId"),
			...
			$service = new PWBillService();
			$this->ajaxReturn($service->selectPOBillListForPWBill($params));
		}
	}
	 ...
	 * 采购入库单中选择采购订单列表 - 物资明细
	 */
	public function poBillDetailListForPWBillSelectPOBill() {
		if (IS_POST) {
			$params = [
					"id" => I("post.id")
			];
			...
			$service = new PWBillService();
			$this->ajaxReturn($service->poBillDetailListForPWBillSelectPOBill($params));
		}
	}
	 ...
	 * 采购入库单中获得采购订单的详情
	 */
	public function getPOBillInfoForPWBill() {
		if (IS_POST) {
			$params = [
					"id" => I("post.id")
			];
			...
			$service = new PWBillService();
			$this->ajaxReturn($service->getPOBillInfoForPWBill($params));
		}
	}
}