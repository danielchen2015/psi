...
class BizConfigDAO extends PSIBaseExDAO
{
     ...
     * @return string
     */
    private function getWSCountLimitName($id)
    {
        switch ($id) {
            case "0" :
                return "不做限制";
     ...
     * @return string
     */
    private function getGoodsCountDecNumberName($id)
    {
        if ($id == "0") {
            return "整数";
        } else {
     ...
     * @return string
     */
    public function getModuleOpenType($companyId)
    {
        $db = $this->db;
     ...
     */
    public function getWSCountLimit($companyId)
    {
        $db = $this->db;
