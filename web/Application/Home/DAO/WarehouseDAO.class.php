...
class WarehouseDAO extends PSIBaseExDAO {
	...
	public function warehouseList($params) {
		...
		$sql = "select w.id, w.code, w.name, w.inited, w.data_org,
					w.org_id, g.full_name 
				from t_warehouse w, t_org g
				where w.org_id = g.id";
	...
	public function addWarehouse(& $params) {
		...
		if (! $org) {
			return $this->bad("选择的组织机构不存在");
		}
		
		$id = $this->newId();
		$params["id"] = $id;
		
		$sql = "insert into t_warehouse(id, code, name, inited, py, data_org, company_id, org_id)
					values ('%s', '%s', '%s', 0, '%s', '%s', '%s', '%s')";
		$rc = $db->execute($sql, $id, $code, $name, $py, $dataOrg, $companyId, $orgId);
		if ($rc === false) {
	...
	public function updateWarehouse(& $params) {
		...
		$sql = "update t_warehouse
				set code = '%s', name = '%s', py = '%s', org_id = '%s'
				where id = '%s' ";
		$rc = $db->execute($sql, $code, $name, $py, $orgId, $id);
		if ($rc === false) {
			return $this->sqlError(__METHOD__, __LINE__);
		}
