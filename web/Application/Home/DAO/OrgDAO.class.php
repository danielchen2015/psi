...
class OrgDAO extends PSIBaseExDAO {
	...
	public function addOrg(& $params) {
		...
		if ($parentId == null) {
			...
			$sql = "insert into t_org (id, name, full_name, org_code, parent_id, data_org, org_type, create_user_id, create_dt)
					values ('%s', '%s', '%s', '%s', null, '%s', %d, '%s', now())";
		...
		if ($parentId == null) {
			...
			$sql = "update t_org
					set name = '%s', full_name = '%s', org_code = '%s', parent_id = null,
						org_type = '%d', update_user_id = '%s', update_dt = now()
					where id = '%s' ";
			$rc = $db->execute($sql, $name, $fullName, $orgCode, $orgType, $loginUserId, $id);
			if ($rc === false) {
				return $this->sqlError(__METHOD__, __LINE__);
			}
		} else {
			...
			if ($data) {
				...
				$sql = "update t_org
						set name = '%s', full_name = '%s', org_code = '%s', parent_id = '%s',
							org_type = '%d', update_user_id = '%s', update_dt = now()
						where id = '%s' ";
				$rc = $db->execute($sql, $name, $fullName, $orgCode, $parentId, $orgType, 
						$loginUserId, $id);
				if ($rc === false) {
					return $this->sqlError(__METHOD__, __LINE__);
				}
	...
	public function getOrgById($id) {
		...
		$sql = "select name, org_code, org_type from t_org where id = '%s' ";
		$data = $db->query($sql, $id);
		if (! $data) {
			return null;
		}
	...
	public function allOrgs($params) {
		...
		$sql = "select id, name, org_code, full_name, data_org, org_type, create_user_id,
		...
		foreach ( $orgList1 as $i => $org1 ) {
			...
			$result[$i]["orgTypeCode"] = $org1["org_type"];
			$result[$i]["createUserName"] = $this->getUserNameById($org1["create_user_id"]);
			$result[$i]["createDT"] = $org1["create_dt"];
			$result[$i]["updateUserName"] = $this->getUserNameById($org1["update_user_id"]);
			$result[$i]["updateDT"] = $org1["update_dt"];
	...
	private function allOrgsInternal($parentId, $db) {
		...
		foreach ( $data as $i => $v ) {
			...
			$result[$i]["orgTypeCode"] = $v["org_type"];
			$result[$i]["createUserName"] = $this->getUserNameById($v["create_user_id"]);
			$result[$i]["createDT"] = $v["create_dt"];
			$result[$i]["updateUserName"] = $this->getUserNameById($v["update_user_id"]);
			$result[$i]["updateDT"] = $v["update_dt"];
