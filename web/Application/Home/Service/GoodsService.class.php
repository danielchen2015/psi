...
class GoodsService extends PSIBaseExService {
	private $LOG_CATEGORY_GOODS = "基础数据-物资";
	private $LOG_CATEGORY_UNIT = "基础数据-物资计量单位";
	private $LOG_CATEGORY_BRAND = "基础数据-物资品牌";
	...
	public function editGoods($params) {
		...
		if ($id) {
			...
			$log = "编辑物资: 编码 = {$code}, 品名 = {$name}, 规格型号 = {$spec}";
		} else {
			// 新增
			...
			$log = "新增物资: 编码 = {$code}, 品名 = {$name}, 规格型号 = {$spec}";
		}
	 ...
	 * 商品字段，查询数据
	 */
	public function queryDataWithPurchasePrice($queryKey, $billType) {
		if ($this->isNotOnline()) {
			return $this->emptyResult();
		}
	...
	public function goodsUnitGroupList($params) {
		...
		return $dao->goodsUnitGroupList($params);
	}
