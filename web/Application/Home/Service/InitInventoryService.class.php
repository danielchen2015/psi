...
class InitInventoryService extends PSIBaseExService {
	...
	public function initInfoList($params) {
		...
		$sql = "select v.id,g.id as goods_id, g.code, g.name, g.spec, convert(v.balance_count, $fmt) as balance_count, v.balance_price, 
	...
	public function editInitInv($params) {
		...
		$dao = new InitInventoryDAO($db);
		
		$params["dataOrg"] = $this->getLoginUserDataOrg();
		$params["loginUserId"] = $this->getLoginUserId();
		
		$rc = $dao->editInitInv($params);
		if ($rc) {
			$db->rollback();
			return $rc;
		}
	...
	public function goodsDetailList($params) {
		...
		$dao = new InitInventoryDAO($this->db());
		return $dao->goodsDetailList($params);
	}
